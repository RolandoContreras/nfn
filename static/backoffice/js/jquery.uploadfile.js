...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var obj = this;
        var uploadLabel = $('<div style="position:absolute;left:0px;top:0px">' + $(this).html() + '</div>');
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                if(!isFileTypeAllowed(obj, s, files[i].name)) {
                    //if(s.showError) $("<div class='" + s.errorClass + "'>" + s.extErrorStr  + "</div>").html("#erro_mensagens");
                    if(s.showError) $("#erro_mensagens").html("<div class='" + s.errorClass + "'>" + s.extErrorStr  + "</div>");
					setTimeout("some_mensagem()",3000);
                    continue;
                }
                if(!s.allowDuplicates && isFileDuplicate(obj, files[i].name)) {
                    if(s.showError) $("<div class='" + s.errorClass + "'>" + s.duplicateErrorStr + "</div>").html("#erro_mensagens");
                    continue;
                }
                if(s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    //if(s.showError) $("<div class='" + s.errorClass + "'>" + s.sizeErrorStr  + "</div>").html(
                    if(s.showError) $( "#erro_mensagens").html("<div class='" + s.errorClass + "'>" + s.sizeErrorStr  + "</div>");
					setTimeout("some_mensagem()",3000);
                    continue;
                }
                if(s.maxFileCount != -1 && obj.selectedFiles >= s.maxFileCount) {
                    if(s.showError) $("<div class='" + s.errorClass + "'>" + s.maxFileCountErrorStr + "</div>").html(
                        "#erro_mensagens");
                    continue;
                }
                obj.selectedFiles++;
